...
func makeTestWatcher(t *testing.T) *Watcher {
	...
	return &Watcher{
		config: &config.AgentConfig{
			...
			Checkpoint: config.CheckpointSpec{
				BasePath: t.TempDir(),
			},
		},
...
func TestHandleCheckpointPodEvent(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.hash != "" {
				labels[config.KubeLabelCheckpointHash] = tc.hash
			}

			var annotations map[string]string
			if tc.annotation != "" {
				annotations = map[string]string{
					config.KubeAnnotationCheckpointStatus: tc.annotation,
				}
			}
...
func TestHandleRestorePodEvent(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.annotation != "" {
				annotations = map[string]string{
					config.KubeAnnotationRestoreStatus: tc.annotation,
				}
			}
			...
			if tc.createDir && tc.hash != "" {
				dir := filepath.Join(w.config.Checkpoint.BasePath, tc.hash)
